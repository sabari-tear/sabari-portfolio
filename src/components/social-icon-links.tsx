...
type SocialIconLinksProps = {
  includeFollowLabel?: boolean;
};

export function SocialIconLinks({ includeFollowLabel = false }: SocialIconLinksProps) {
  const hrefByLabel = new Map(socialLinks.map((social) => [social.label, social.href]));

  return (
    <ul className="ml-1 mt-8 md:mb-8 flex items-center">
      {includeFollowLabel ? (
        <li className="mr-5 text-xs uppercase">
          <h3>{siteContent.labels.followMe}</h3>
        </li>
      ) : null}
      {socials.map((social) => (
